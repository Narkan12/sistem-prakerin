        ...
        private void TampilData()
        {
            ...
            foreach (DataRow row in Koneksi.ds.Tables[0].Rows)
            {
                DGVRole.Rows.Add(
                    ...
                    row["nama"].ToString(),
                    row["tanggal"] == DBNull.Value
                        ? ""
                        : Convert.ToDateTime(row["tanggal"]).ToString("yyyy-MM-dd"),
                    row["jam_masuk"] == DBNull.Value
                        ? ""
                        : row["jam_masuk"].ToString(),
                    row["jam_keluar"] == DBNull.Value
                        ? ""
                        : row["jam_keluar"].ToString(),
                    row["status"].ToString()
                );
            }

            //button enabled dan disabled
            btnEdit.Enabled = false;
            btnHapus.Enabled = false;
        }
